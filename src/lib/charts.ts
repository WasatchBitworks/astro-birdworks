...
export function renderDailyChart(container: Element, data: Array<{ date: string; count: number }>): void {
  ...
  data.forEach((d, i) => {
    ...
    const bar = svgRect(x, y, barWidthActual, barHeight, "#3E6A21");
    bar.setAttribute("rx", "2");
    svg.appendChild(bar);

    if (d.count > 0) {
      svg.appendChild(svgText(x + barWidthActual / 2, y - 5, String(d.count), "12px", "#463f33", "middle"));
    }
...
export function renderHourlyAverage(container: Element, detections: Array<{ detected_at: string }>): void {
  ...
  hourlyAverages.forEach((hourData, i) => {
    ...
    const bar = svgRect(x, y, barWidth, barHeight, "#3E6A21");
    bar.setAttribute("rx", "2");
    svg.appendChild(bar);

    const hourLabel = svgText(padding.left - 10, y + barHeight / 2, hourData.label, "11px", "#463f33", "end");
    ...
    if (hourData.average > 0) {
      ...
      const countLabel = svgText(x + barWidth + 5, y + barHeight / 2, String(avgRounded), "11px", "#463f33", "start");
...
export function renderExtendedTimeline(container: Element, data: Array<{ label: string; value: number }>): void {
  ...
  data.forEach((d, i) => {
    ...
    if (d.value > 0 && barWidth > 15) {
      svg.appendChild(svgText(x + barWidthActual / 2, y - 5, String(d.value), "10px", "#463f33", "middle"));
    }
...
export function renderSpeciesComparison(container: Element, data: Array<{ label: string; value: number }>): void {
  ...
  data.forEach((d, i) => {
    ...
    const bar = svgRect(x, y, barWidth, barHeight, "#4A7A9B");
    bar.setAttribute("rx", "2");
    svg.appendChild(bar);

    const nameLabel = d.label.length > 20 ? d.label.substring(0, 18) + "..." : d.label;
    const label = svgText(padding.left - 10, y + barHeight / 2, nameLabel, "11px", "#463f33", "end");
    label.setAttribute("dominant-baseline", "middle");
    svg.appendChild(label);

    const countLabel = svgText(x + barWidth + 5, y + barHeight / 2, String(d.value), "11px", "#463f33", "start");
    countLabel.setAttribute("dominant-baseline", "middle");
    svg.appendChild(countLabel);
  });
...
/** Heatmap color ramp: light sage → forest green → ochre → rust */
export function getHeatmapColor(intensity: number): string {
  if (intensity === 0) return "#f7f4ea";

  const colorStops = [
    { threshold: 0.0, color: { r: 236, g: 243, b: 225 } },
    { threshold: 0.25, color: { r: 194, g: 212, b: 168 } },
    { threshold: 0.5, color: { r: 114, g: 154, b: 76 } },
    { threshold: 0.65, color: { r: 62, g: 106, b: 33 } },
    { threshold: 0.75, color: { r: 185, g: 127, b: 36 } },
    { threshold: 0.85, color: { r: 168, g: 90, b: 30 } },
    { threshold: 1.0, color: { r: 140, g: 58, b: 28 } },
  ];
...
export function renderDailySummary(
  ...
  svg.appendChild(
    svgText(padding.left, 45, `Last updated: ${toMountainTime(updatedStr)} MT`, "12px", "#6d6457", "start"),
  );

  const divider = svgLine(padding.left, headerHeight, width - padding.right, headerHeight, "#e6dfd0");
  ...
  data.species.forEach((species, i) => {
    ...
    const nameText = svgText(padding.left - 10, y + rowHeight / 2, nameLabel, "12px", "#463f33", "end");
    ...
    const bar = svgRect(padding.left, barY, barWidth, barH, "#3E6A21");
    bar.setAttribute("rx", "3");
    svg.appendChild(bar);

    if (species.total > 0) {
      const countText = svgText(padding.left + barWidth + 5, y + rowHeight / 2, String(species.total), "11px", "#463f33", "start");
    ...
    species.hourly.forEach((count, hour) => {
      ...
      if (count > 0) {
        ...
        if (cellSize >= 24) {
          const textColor = intensity > 0.5 ? "#ffffff" : "#463f33";
        ...
        const cell = svgRect(cellX, cellY, cellSize - 2, cellH, "#f7f4ea");
        cell.setAttribute("rx", "2");
        cell.setAttribute("stroke", "#e6dfd0");
  ...
  for (let hour = 0; hour < 24; hour += 3) {
    ...
    svg.appendChild(svgText(hourX, hourLabelsY, String(hour), "10px", "#6d6457", "middle"));
  }

  const hourAxisLabel = svgText(heatmapStartX + heatmapWidth / 2, hourLabelsY + 15, "Hour of Day (MT)", "11px", "#6d6457", "middle");
